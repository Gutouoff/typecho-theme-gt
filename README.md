...
| 设置项 | 说明 |
| --- | --- |
| 刊头期号 | 首页刊头左上角小标签，如 `ISSUE 001` |
| 刊头副标题 | 首页刊头斜体简介；留空用「站点描述」 |
| 首页头条大卡 | 开启后首页第一篇显示为通栏头条卡，其余为编号卡片 |
| 上一篇 / 下一篇 | 文章页底部导航开关 |
| 文章标签 | 文章页标签贴纸开关 |
| 站点副标题 | 显示在页脚；留空时使用「站点描述」 |
| 社交链接 | 每行一条 `名称|URL|简介`（简介可省略），显示在页脚 |
| 友情链接 | 每行一条 `名称|URL|简介`，在「友情链接」页面模板中渲染为名片卡片 |
| 页脚附加文本 | 如备案号，可为空 |
| 代码高亮 | 是否加载 highlight.js（自托管）对代码块高亮 |
| 自定义 CSS | 追加到页面的自定义样式（无需 `<style>` 标签） |
| 头部自定义代码 | 输出在 `</head>` 前，可放统计代码 / meta |
